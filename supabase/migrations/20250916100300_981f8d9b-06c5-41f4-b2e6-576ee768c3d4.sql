-- Criar sistema completo de notificações e candidaturas

-- Primeiro, vamos verificar se existe algum problema com a criação automática de perfis de cliente
-- Vamos criar uma função para garantir que clientes tenham perfil completo
...
DECLARE
  ...
  montador_nome TEXT;
BEGIN
  -- Buscar informações do job e cliente
  SELECT j.*, c.user_id as cliente_user_id
  INTO job_record, client_user_id
  FROM jobs j
  ...
  VALUES (
    client_user_id,
    'nova_candidatura',
    montador_nome || ' se candidatou para seu pedido: ' || substring(job_record.descricao, 1, 40) || '...'
  );
